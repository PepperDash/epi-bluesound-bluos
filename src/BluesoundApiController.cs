...
namespace PepperDash.Essentials.Plugin
{
	...
	public class BluesoundApiDevice : EssentialsBridgeableDevice
	{
		...
		private void RefreshServices()
		{
			...
			{
				RefreshServicesWithDefault(defaultService);
				return;
			}
		...
		private void RefreshServicesWithDefault(string defaultService)
		{
			if (!BrowseServices(null))
				return;

			var match = allServices.FirstOrDefault(s =>
				string.Equals(s.Name, defaultService, StringComparison.OrdinalIgnoreCase));

			if (match == null || string.IsNullOrEmpty(match.BrowseKey))
			{
				this.LogWarning("DefaultService '{name}' not found in root browse — showing all services", defaultService);
				preferredServiceBrowseKey = null;
			...
			preferredServiceBrowseKey = match.BrowseKey;
			preferredServiceName = match.Name;
			browseKeyStack.Clear();
			browseNameStack.Clear();

			if (BrowseServices(preferredServiceBrowseKey))
			{
				browseKeyStack.Push(preferredServiceBrowseKey);
				browseNameStack.Push(match.Name);
				currentServicesMenu = match.Name;
			}
		...
		/// <summary>Reset the services list to the root /Browse level (or preferred service root)</summary>
		public void ServiceHomePage()
		{
			...
			receiveQueue.Enqueue(new CommandMessage(() =>
			{
				...
				servicePageIndex = 0;
				FireServiceFeedbacks();
				if (!string.IsNullOrEmpty(preferredServiceBrowseKey))
				{
					browseKeyStack.Clear();
					browseNameStack.Clear();
					if (BrowseServices(preferredServiceBrowseKey))
					{
						browseKeyStack.Push(preferredServiceBrowseKey);
						browseNameStack.Push(preferredServiceName);
						currentServicesMenu = preferredServiceName;
					}
				}
				else
				{
		...
		// Returns 1 when a preferred service is active so Home/Back hide at the service root
		private int PreferredServiceDepth => string.IsNullOrEmpty(preferredServiceBrowseKey) ? 0 : 1;
